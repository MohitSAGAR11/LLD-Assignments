...
1. `services.CafeteriaSystem.checkout` mixes menu lookup, pricing, tax rules, discount rules, printing, and persistence.
...
Checkpoint C: Move invoice formatting out of `services.CafeteriaSystem`.
...
- Keep `entities.MenuItem` and `entities.OrderLine` public fields unchanged.
- No external libraries.

## 6. Acceptance criteria
- `services.CafeteriaSystem` orchestrates only; it should not format strings or encode tax/discount specifics.
